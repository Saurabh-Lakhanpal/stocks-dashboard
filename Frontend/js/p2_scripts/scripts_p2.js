...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchSP500Data(ticker, startDate, endDate) {
        const url = ticker ? `http://127.0.0.1:5000/api/v1.0/sp500?ticker=${ticker}&start_date=${startDate}&end_date=${endDate}` : `http://127.0.0.1:5000/api/v1.0/sp500?start_date=${startDate}&end_date=${endDate}`;
        try {
            const response = await fetch(url);
            if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
